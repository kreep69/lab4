...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(5.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 20.0f, new float[]{20, 5, 10, 5, 10, 5, 20, 5, 5, 5}, 0.0f);
...
// Отобразить график
    ...
    double result_y(double y)
    {
        double value_y=0;
        int counter=0;
        value_y +=y;
        counter++;
        double result =value_y/counter;
        return result;
    }
...
// Определить, сколько места понадобится для надписи "y"
            ...
            Point2D.Double labelPos = xyToPoint(0.1, maxY);
...
// Вывести надпись в точке с вычисленными координатами
        ...
        Point2D.Double labelPos1 = xyToPoint(0, -0.8);
        canvas.drawString("0", (float) (labelPos1.getX() -
                bounds1.getWidth() -10), (float) (labelPos1.getY() + bounds1.getY()-10));
        /*canvas.draw(new Line2D.Double(xyToPoint(-2, 5),
                xyToPoint(2, 5)));
        Rectangle2D bounds = axisFont.getStringBounds("5", context);
        Point2D.Double labelPos = xyToPoint(0, 5);
        canvas.drawString("5", (float) (labelPos.getX() -
                bounds.getWidth() -10), (float) (labelPos.getY() + bounds.getY()-10));*/
    }
